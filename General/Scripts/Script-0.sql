...
-- 계정 생성
CREATE USER SPRING_LKW IDENTIFIED BY SPRING1234;

-- 권한 부여
GRANT CONNECT, RESOURCE TO SPRING_LKW;

-- 테이블 저장 공간 할당
ALTER USER SPRING_LKW
DEFAULT TABLESPACE USERS
QUOTA 100M ON USERS;*/

--------------------------------------------------------------------

/* SPRING 계정 접속후 테이블, 시퀀스 생성 */
...
COMMENT ON COLUMN "MEMBER"."MEMBER_TEL" 			IS '회원 전환번호(-제외)';
...
VALUES(SEQ_MEMBER_NO.NEXTVAL, 'member01@kh.or.kr', 
			'pass01!', '샘플1', '01012341234', NULL, NULL, 
...
BEGIN
	FOR I IN 1..3000 LOOP
		...
		VALUES(
			...
			DEFAULT, DEFAULT,
			6, 
			CEIL (DBMS_RANDOM.VALUE(270, 2006) ),
			NULL 
		);
	END LOOP;
END;
...
-- ROW_NUMBER() OVER (ORDER BY BOARD_NO ASC) AS "RNUM"
 --> BOARD_NO 오른차순으로 정렬 후 조회된 행의 번호를 지정
 --  + 해당 컬럼 별칭을 "RNUM" 지정
 --> 연속되지 않은 PK대신 연속된 숫자를 만들려고 사용
