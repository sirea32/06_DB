...
-- VIEW를 만들때 사용한 원본 테이블에
-- 값이 INSERT 됨을 확인!

	--> 하지만 모든 컬럼 값이 INSERT 된것이 아니라
	--  VIEW를 생성할때 사용된 컬럼에만 값이 INSERT되어
	--  반대로 사용되지 않은 컬럼에는 NULL이 들어감!!
		--> NULL은 D8의 무결성을 약하게 만드는 중요원인!!
...
-- 왜 사용할까?
--> VIEW를 이용해서 DML(INSERT, UPDATE,DELETE) 하지 밀라고!
...
-- TB_TEST 테이블에 PC값을 SEQ_TEST_NO 시퀀스로 생성하기
INSERT INTO TB_TEST
